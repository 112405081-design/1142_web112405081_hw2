 ...
 */
export function QuizPageShell({ variant, children }: Props) {
  return (
    <div className="flex min-h-dvh w-full flex-col items-center justify-center bg-stone-300/80 px-3 py-3 dark:bg-stone-950 sm:px-4 sm:py-6">
      <main
        className="flex w-full flex-1 flex-col items-center justify-center pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]"
      >
        {/* 直式比例：寬度優先，高度約 9:16，不超過視窗 */}
        <article
        ...
        >
          ...
          <div
            ...
            className="relative z-10 flex min-h-0 flex-1 flex-col justify-center overflow-x-hidden overflow-y-auto overscroll-contain px-4 py-5 text-stone-900 sm:px-5 sm:py-6"
          >
            {children}
          </div>
        </article>
